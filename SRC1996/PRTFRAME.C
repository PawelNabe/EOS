...
static void printMetering(metbuf)
char *metbuf;
{
	if (userCharsDefined) {
		dWidOnOff(1);
...
static void prMargin(tb, inch, endc)
char *tb, *inch, endc;
{
	printf("	%s Margin =		%s Inches%c\n", tb, inch, endc);
} /* prMargin */

static void prTopMargin(inch)
char *inch;
{
	prMargin("Top", inch, ',');
} /* prTopMargin */
